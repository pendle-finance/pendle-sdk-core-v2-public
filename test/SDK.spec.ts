...
describe(SDK, () => {
    ...
    it('#userPositionPY',async () => {
        const userPositionPY = await sdk.getUserPYPositionsByPYs(currentConfig.deployer,[currentConfig.ytAddress,currentConfig.ptAddress]);
        expect(userPositionPY).toBeDefined();
    })

    it('#userPositionMarket',async () => {
        const userPositionMarket = await sdk.getUserMarketPositions(currentConfig.deployer,[currentConfig.marketAddress]);
