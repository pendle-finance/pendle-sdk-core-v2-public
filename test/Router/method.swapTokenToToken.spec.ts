...
describe(pendleSDK.Router.prototype.swapTokenToToken, () => {
    const pairsTokenToTest = [
        { fromToken: tokenHelper.NATIVE_TOKEN_0x00, toToken: testEnv.currentConfig.zappableTokensToTest[0] },
        { fromToken: testEnv.currentConfig.zappableTokensToTest[0], toToken: tokenHelper.NATIVE_TOKEN_0x00 },
    ];
    ...
    describe.each(pairsTokenToTest)(nameTemplate, ({ fromToken, toToken }) => {
        ...
        beforeAll(async () => {
            metaMethod = await router.swapTokenToToken(input, toToken.address, constants.SLIPPAGE_TYPE2, {
                method: 'meta-method',
            });
        });
        ...
        describe('sent tx', () => {
            ...
            it('should have user balance transffered correctly', async () => {
